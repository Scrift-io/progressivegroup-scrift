...
const Chatbot = () => {
  ...
  const [messages, setMessages] = useState([{
    type: 'bot',
    text: 'Hello! Welcome to Progressive Group. How can I help you today?'
  }]);
  ...
  const predefinedResponses: {
    ...
    'contact': 'You can reach us at L-66, LDA Industrial Area, Lahore, Pakistan. Phone: +92-123-4567890, Email: info@pgtraders.com',
  ...
  return <>
      ...
      {/* Chatbot Window */}
      {isOpen && <div className="fixed bottom-24 right-6 z-50 w-80 h-96 bg-white rounded-lg shadow-2xl border border-gray-200 flex flex-col animate-scale-in">
          {/* Header */}
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3">
            {messages.map((message, index) => <div key={index} className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-xs p-3 rounded-lg text-sm ${message.type === 'user' ? 'bg-pg-red text-white' : 'bg-gray-100 text-gray-800'}`}>
                  {message.text}
                </div>
              </div>)}
          </div>
          ...
          <div className="p-4 border-t border-gray-200">
            <div className="flex space-x-2">
              <input type="text" value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyPress={handleKeyPress} placeholder="Type your message..." className="flex-1 p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pg-red focus:border-transparent" />
              <button onClick={handleSend} className="bg-pg-red text-white p-2 rounded-lg hover:bg-red-700 transition-colors duration-300">
